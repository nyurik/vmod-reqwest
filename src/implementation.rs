...
use varnish::vcl::{log, Ctx, Event, LogTag, Probe, Request as ProbeRequest, VclError, VclResult};
    ...
    fn get_headers(&self, ctx: &mut Ctx<'_>) -> VclResult<Option<BackendResp>> {
        ...
        for (k, v) in &resp.headers {
            beresp.set_header(
                k.as_str(),
                v.to_str()
                    .map_err(|e| <std::string::String as Into<VclError>>::into(e.to_string()))?,
            )?;
        }
        Ok(Some(BackendResp {
            bytes: None,
